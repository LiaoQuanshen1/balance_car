...
float TurnMaxAngle = 75;   // 大弯道转向参数
float TurnMidAngle = 40;   // 中等转向参数（丢线时使用）
float TurnMinAngle = 30;   // 微调转向参数
// 速度参数
float BaseSpeed = 500;      // 基础巡线速度（直行时的速度）
float ForwardLimit = 600;		//前行限制(转向大于该值限制其前进)
// ===== 传感器状态定义--识别到黑线时为1 =====
typedef enum {
...
#define FINISH_LAPS 5u          /* 经过终点次数达到该值才停车（竞速圈数，可调） */
...
#endif
static u8 lap_count = 0;                /* 已识别终点次数：达到 FINISH_LAPS 才停车 */
